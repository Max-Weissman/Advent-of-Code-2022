...
class dir {
    ...
    findValue(){
        let tracker = this
        while( !this.checked || tracker.parent){ //if at root and calculated all the files
            if (!tracker.checked){ //if haven't checked this node yet
                let checkChild = false
                tracker.folders.forEach(child => { //see which children have been checked
                    if (!child.checked){ 
                        checkChild = child
                    }
                })
                if (checkChild){ //move to an unchecked child
                    tracker = checkChild
                }
                else{ //otherwise this has been checked
                    tracker.checked = true
                }
            }
            else if (tracker.parent){ //increase the parents value by this once checked then move back to the parent
                tracker.parent.files += tracker.files
                tracker = tracker.parent
            }
        }
        let size = Infinity
        let neededSpace = tracker.files + 30000000 - 70000000
        while(tracker.folders.length > 0 || tracker.parent){ //if at root and calculated all the files
            if (tracker.folders.length > 0){
                ...
                if (tracker.files >= neededSpace && tracker.files < size){
                    size = tracker.files
                }
                tracker = tracker.parent
                tracker.folders.shift() //delete compared nodes to the value needed
            }
        }
        return size
    }
